...
cdef export_cluster_results(
    ...
    """
    cdef str cluster_parquet_filename = config.output_filename+'.parquet'
...
cdef int _precursor_to_interval(
    mz: float,
    charge: int,
    interval_width: int
    ):
    ...
    int
        ...
        charge belongs.
    """
    cdef float hydrogen_mass, cluster_width, neutral_mass
    hydrogen_mass = 1.00794
    cluster_width = 1.0005079
...
cdef _get_mz_mask(
    ...
    cdef int mz_size = mz.size
    cdef Py_ssize_t i 
    for i in range(mz_size):
        if(mz[i]<min_mz or mz[i]>max_mz):
            mask[i] = False
    return mask

cdef list _set_mz_range(
    spectrum: list,
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef bint _check_spectrum_valid(
    spectrum_mz: float[:],
    min_peaks: int,
    min_mz_range: float
    ):
...
cdef double mass_diff(double mz1, double mz2, bint mode_is_da):
    """
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef mass_diff_mask(mz, remove_mz, tol, bint mode_is_da):
    """
    Calculate the mass difference(s).
    Parameters
    ----------
...
cdef list _remove_precursor_peak(
        spectrum: list,
    ...
    cdef float adduct_mass, neutral_mass, c_mass_diff
...
cdef preprocess_read_spectra_list(
        list spectra_list, 
        int min_peaks = 5, float min_mz_range = 250.0,
        int mz_interval = 1,
    ...
    for i in range(len(spectra_list)):
        ...
        spectra_list[i][7] = _scale_intensity(spectra_list[i][7], scaling, max_intensity=None, degree=2, base=2, max_rank=max_peaks_used)
...
def load_process_spectra_parallel(
    ...
    with Parallel(n_jobs=config.cpu_core_preprocess) as parallel_pool:
        read_spectra_list = parallel_pool(delayed(load_process_single)(
                file = f_i, 
